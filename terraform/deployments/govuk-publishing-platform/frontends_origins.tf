...
module "www_frontends_origin" {
  ...
  name                                 = "www-fe-${var.govuk_environment}-${local.workspace}"
  vpc_id                               = local.vpc_id
  ...
  extra_aliases                        = local.is_default_workspace ? ["www.${var.publishing_service_domain}", "www.${local.workspace_external_domain}"] : []
...
module "draft_frontends_origin" {
  ...
  name                                 = "draft-fe-${var.govuk_environment}-${local.workspace}"
  vpc_id                               = local.vpc_id
  aws_region                           = data.aws_region.current.name
  assume_role_arn                      = var.assume_role_arn
  public_subnets                       = local.public_subnets
