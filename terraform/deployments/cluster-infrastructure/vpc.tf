...
resource "aws_subnet" "eks_control_plane" {
  ...
  tags = {
    Name = "${local.cluster_name}-eks-control-plane-${each.key}"
  }
}
...
resource "aws_subnet" "eks_public" {
  ...
  availability_zone = each.value.az
  tags = {
    Name = "${local.cluster_name}-eks-public-${each.key}"
...
resource "aws_eip" "eks_nat" {
  ...
  vpc      = true
  tags = {
    Name = "${local.cluster_name}-eks-nat-${each.key}"
  }
...
resource "aws_nat_gateway" "eks" {
  ...
  subnet_id     = aws_subnet.eks_public[each.key].id
  tags = {
    Name = "${local.cluster_name}-eks-${each.key}"
  }
  # TODO: depends_on = [aws_internet_gateway.gw] once we've imported the IGW from govuk-aws.
}
...
resource "aws_subnet" "eks_private" {
  ...
  availability_zone = each.value.az
  tags = {
    Name = "${local.cluster_name}-eks-private-${each.key}"
    # https://docs.aws.amazon.com/eks/latest/userguide/alb-ingress.html
    "kubernetes.io/cluster/${local.cluster_name}" = "owned"
